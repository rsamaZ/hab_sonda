...
# lleva a bordo. Carga una serie de reglas definidas en el archivo de configuraci�n hav.conf y aplica dichas
# reglas a los datos recuperados. La salida es un archivo ruledata.log donde figuran las acciones que otros componentes
# pueden hacer en funci�n de estas reglas (aumentar frecuencias de envio, encender y apagar sistemas, etc.)
...
#
# Las reglas se separan unas de otras en la configuraci�n por comas (,) simples
...
#
# Ejemplo1:
# Regla que informa que si el campo 0 de los datos del gps (altura) es mas peque�a de 10000 metros se informara
...
def sensorLogFile(idSensor):
    ...
    loggerLog.debug(
        "[RuleService][sensorLogFile] Archivo de sensores recuperado: " + str(path))
    return path
...
def getRuleSensor(rule):
	...
	try:
		...
		loggerLog.error(
		    "[RuleService][getRuleSensor] Se ha producido un error parseando la regla")
		return "NORULE"
...
def executeRule(ruleComponent):
	...
	try:
		...
		if lenRuleFields == 9:
			createTokenContent = ruleComponent[5] + ":" + \
			    ruleComponent[6] + ":" + ruleComponent[8] + ":"
		else:
			if lenRuleFields == 7:
		...
		loggerLog.error(
		    "[RuleService][executeRule] Se ha producido un error ejecutando la regla")
	...
	try:
		...
		if operationToEvaluate == "lt":
			if dataToEvaluate < thresholdToEvaluate:
				loggerLog.warn(
				    "[RuleService][evaluateRule] Aplicando regla [" + str(rule) + "]")
				# Se aplica la regla
				...
				loggerLog.warn(
				    "[RuleService][evaluateRule] OJO, no se puede evaluar la comparacion!")

	except:
		loggerLog.error("[RuleService][getRuleSensor] Exception: " + str(e))
		loggerLog.error(
		    "[RuleService][getRuleSensor] Se ha producido un error evaluando la regla [" + str(rule) + "]")
				...
				if (os.path.exists(pathSensor)):
					...
					if (isSensorFileFull):
						...
						for data in dataSensorArray:
							...
							i = i + 1
							loggerLog.debug(
							    "[RuleService][datosProcesados: " + datosProcesados + "]")
						f.close()
